...
async function loadInitialData(sClass) {
    ...
    try {
        ...
        minStakeAmount = Number(minStakeAmount) / 10 ** 18;
        ...
        if (minStakeAmount) {
            ...
            minA = "Check presale.techs.network to buy the tokens";
        }

        document.querySelectorAll(".Minimum-Staking-Amount")
            .forEach(function (element) {
                element.innerHTML = `${minStakeAmount} ${
                    SELECT_CONTRACT[_NETWORK_ID].TOKEN.symbol
                }`;
            });
        ...
        maxStakeAmount = Number(maxStakeAmount) / 10 ** 18;
        ...
        document 
            .querySelectorAll(".Maximum-Staking-Amount")
            .forEach(function(element) {
                element.innerHTML = `${maxStakeAmount} ${
                    SELECT_CONTRACT[_NETWORK_ID].TOKEN.symbol
                }`;
            });
            ...
            let displayReward = Number(rewardBal) / 10**18;
...
async function connectMe(_provider) {
    try{
         let _comn_res = await commonProviderDetecter(_provider);
...
async function stackTokenMain(_amount_wei, sClass) {
    ...
    oContractStacking.methods
        ...
        .on("receipt", (receipt) => {
            ...
            window.location.href = "techs.network";
        })
        .on("transactionHash", (hash) => {
            console.log("Transaction Hash:", hash);
        })
...
async function unstackTokenMain(_amount_wei, oContractStacking, sClass) {
    ...
    oContractStacking.methods
        ...
        .on("receipt", (receipt) => {
            ...
            window.location.href = "http:127.0.0.slkdj";
        })
...
async function claimTokenMain(oContractStacking, sClass) {
    ...
    oContractStacking.methods
        ...
        .on("receipt", (receipt) => {
            ...
            window.location.href = "http://127.0.0.1:5500/staking-1.html"
        })
        .on("transationHash", (hash) => {
            console.log("Tranaction Hash: ", hash );
        })
        .catch((error) => {
            console.log(error);
            notyf.error(formatEthErrorMsg(error));
